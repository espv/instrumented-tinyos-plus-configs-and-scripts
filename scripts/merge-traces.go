...
func main() {
	static_traces, err := os.Open("static-traces.txt")
	check(err)

	defer static_traces.Close()

	var last_time int = 0
	scanner := bufio.NewScanner(static_traces)
	var traces bytes.Buffer
	traces.WriteString("EOD\n")
	i := 0
	for scanner.Scan() {
		...
		//fmt.Println(i)
		fmt.Println(eid)
		traces.WriteString(eid_to_event(eid, cycles))
		last_time = cycles
	}
	// Trace 47 ends the task loop and service
	traces.WriteString(eid_to_event(47, last_time))
	traces.WriteString("H		\n")

	if err := scanner.Err(); err != nil {
		log.Fatal(err)
	}

	err = ioutil.WriteFile("trace.espen", traces.Bytes(), 0644)
	check(err)
}
